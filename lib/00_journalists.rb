...
puts "Il y a #{Journalist.count} handles dans cette array"
puts "---------------#############---------------"

# Quel est le handle le plus court de cette liste ?
longJourn = Journalist[0].length
Journalist.each  {
    |journ|
    if journ.length <= longJourn
        longJourn = journ.length
        $shortJourn = journ
    end
}
puts "Le handle le plus court est #{$shortJourn}"
puts "---------------#############---------------"
...
Journalist.each  {
    |journ|
    if journ.length == 6
        $fiveJourn = $fiveJourn + 1
    end
}
puts "Il y a #{$fiveJourn} handles de 5 caractères (le @ ne compte pas pour un caractère)"
puts "---------------#############---------------"
...
Journalist.each  {
    |journ|
    if journ.slice(1) =~ /[A-Z]/
        $majJourn = $majJourn + 1
    end
}
puts "Il y a #{$majJourn} qui commencent par une majuscule (première lettre juste après le @)"
puts "---------------#############---------------"

#Trie la liste de handle par ordre alphabétique.
puts "Trie la liste de handle par ordre alphabétique"
journalistA=Journalist
journalistA.sort.each do |journ|
    puts journ
end
puts "---------------#############---------------"

# Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
journalistT=Journalist
puts "Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)"
puts journalistT.sort_by! { |n| n.size }
puts "---------------#############---------------"

#Quelle est la position dans l'array de la personne @epenser ?
puts "@epenser est à la position #{Twitter.index('@epenser')} dans l'array"
puts "---------------#############---------------"
...
for repartition in 4..$maxJourn.length
    $tab=[]
    Journalist.each  {
    |journ|
    if journ.length == repartition
        $tab.push(journ)
    end
    }
    if $tab.length>0
    puts "handle à #{repartition} caractère(s)"
    puts $tab
    puts "------------------------"
    end
end
